...
lazy_static! {
    static ref ALL_2020_PROBLEMS: HashMap<DayPart, SolveFn> = {
...
pub fn solve_part(day: u32, part: u32) -> SolveResult<String> {
    ...
    let res = match ALL_2020_PROBLEMS.get(&day_part) {
        Some(f) => f(),
        None => Err(AdventError::NotImplemented(2020, day, part)).map_err(anyhow::Error::msg),
    };
